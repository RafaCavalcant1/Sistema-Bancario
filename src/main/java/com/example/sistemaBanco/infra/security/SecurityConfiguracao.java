...
public class SecurityConfiguracao {
	...
	SecurityFilterChain securityFilterChain(HttpSecurity httpSecurity) throws Exception {
		...
		return httpSecurity.csrf(csrf -> csrf.disable()) // para desligar essa config
				...
				// STATELESS(auetnticação via token) não guarda quem ta logado
				...
				.authorizeHttpRequests(authorize -> authorize
						...
						// qualquer request que seja do tipo post p endoint usuarios, o usuario pode ser
						// Comum
						...
						// Permitir que apenas usuários COMUM faca trnasfe
	...
	@Bean
	// esse método é um factory method para criar e configurar um bean do tipo
	// AuthenticationManager
	// AuthenticationConfiguration é uma classe fornecida pelo Spring Security que
	// tem a configuração de autenticação padrão.
